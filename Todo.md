...
1. ~~تنفيذ قرارات 2026-08-20~~ ✅ **منجز** (commit قادم بعد المراجعة): `src/scheduler.ts` + `src/index.ts` + `db/schema.sql` + `test/*` — 33 اختبارًا خضراء + `typecheck` نظيف. **المتبقي قبل النشر: تطبيق ترحيل الإنتاج `db/migrations/0001_clients_global_rules.sql` (9 أعمدة جديدة بـ DEFAULT) بموافقة المشغل، ثم النشر الحي.**
