...
- **المسار الوحيد الحقيقي (Playwright Wizard)**: بعد اكتشاف موعد (`SLOTS`) وإعادة التحقق من الشبكة قبل الإطلاق، يفتح `executePlaywrightFallback` (مع `CAPTCHA_API_KEY` من `src/captcha.ts`) متصفحًا ويمر بالمعالج الكامل: Office=KAIRO → CalendarId → PersonCount=1 → صفحة المعلومات → شبكة الأسبوع (Radios: 10:00/10:30/11:00/11:30 — London evidence Wed 8/26) → اختيار الموعد → نموذج البيانات بـ31 حقلًا حقيقيًا (Lastname/Firstname/DateOfBirth/TraveldocumentNumber/Sex/Postcode/Telephone/DSGVOAccepted …) + حقول BDC_* المخفية + `CaptchaText` محلولًا عبر 2captcha (C65P عينة، 4-char ديناميكي) → Submit → تحليل `GESX-...`. النجاح فقط عند وجود المرجع، مع حد 20 ثانية بين الإطلاقات (NFR-2).
