...
3. Responsive SPA (embedded in Worker `src/index.ts:690 getAdminHTML()`, not separate Static Assets) loads in under 1 second; auth is Access edge `302` + Worker `opran_admin_token` HttpOnly cookie `src/index.ts:119` — Service Token only for external M2M, never for panel.
