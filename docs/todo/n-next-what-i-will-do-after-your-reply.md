...
3. ~~Panel + API protection~~ ✅ Done 2026-08-20/21: Access enabled + `ADMIN_API_KEY` installed — panel works via Basic dialog (→ session cookie) or `X-API-Key` after Access login; hardened 2026-08-21 (keyed-hash cookie, constant-time compares, no `?token`, logout, security headers).
