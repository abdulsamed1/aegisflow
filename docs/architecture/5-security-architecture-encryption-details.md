...
**CORS:** `Allow-Origin: *` + `Allow-Headers: Content-Type` only — cookie auth is SameSite=Strict same-origin, so `*` never exposes credentials (deferred pinning to panel origin, ponytail).
