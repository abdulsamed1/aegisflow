...
- **Verified path is the browser wizard only** (`docs/portal-automation-spec.md §6` revised 2026-08-21, London evidence; launcher migrated 2026-09-11 from `@cloudflare/playwright` to `@cloudflare/puppeteer` — playwright-core's `_connectOverCDPInternal` unconditionally calls `fs.promises.mkdtemp`, unimplemented on Workers, so every launch crashed; puppeteer's acquire→connect path over `binding.fetch` + WebSocket is fs-free; injectable `opts.launcher` keeps it unit-testable). `executePlaywrightFallback` navigates Office→CalendarId→PersonCount→Info→Week grid radios→Slot→Personal data (31 real names `Lastname/Firstname/DateOfBirth/TraveldocumentNumber/Sex/Postcode/Telephone/DSGVOAccepted` + `BDC_*/CaptchaText` + `Token/StartTime`) and extracts `GESX-...`; success only with reference. Submit targets `input#nextButton` (verified live 2026-08-22 — generic `input[type=submit]` hits Back). The `~200ms` figure in prior revisions applied only to the scanner POST (`FR-5`), not booking — wizard wall time is ~35-50s including captcha solve and the 20s launch-spacing gate (NFR-2); billed browser seconds exclude the gate wait since 2026-09-11 (`workStartTime`).
- **Autonomous Submission**: browser wizard submits via `@cloudflare/puppeteer` (`launchBrowser`), solves CAPTCHA via `src/captcha.ts` — **audio-first (parallel Whisper models since 2026-08-22)**: BotDetect `get=sound` WAV → concurrent `whisper-tiny-en` + `whisper-large-v3-turbo` (Workers AI binding/REST, free) with image fallback (llama-3.2-11b-vision → REST → tesseract.js); no API key, no paid service (`docs/portal-automation-spec/7-captcha.md`) — and parses `GESX-...`.
