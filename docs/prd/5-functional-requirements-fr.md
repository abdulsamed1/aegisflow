...
- **Autonomous Submission**: Playwright wizard submits via `@cloudflare/playwright`, solves CAPTCHA via `src/captcha.ts` — **audio-first since 2026-08-22**: BotDetect `get=sound` WAV → Whisper ASR (Workers AI binding/REST, free) with image fallback (llama-3.2-11b-vision → REST → tesseract.js); no API key, no paid service (`docs/portal-automation-spec/7-captcha.md`) — and parses `GESX-...`.
